# MCQ Detailed Reference

This document is the detailed reference for the MCQ plugin.
...
MCQ registers a teacher feedback renderer and server handlers. **Review and grade** is available for every submitted MCQ and whole-group review navigates all submitted learners. A teacher can see the submitted choices and author an overall summary without AI; if generated feedback exists, the same form is pre-populated and also exposes its per-question explanations. The revision handler returns feedback only and never a grading result, so **Assess with AI** explicitly says that the deterministic answer-key grade will not change. The shared review wrapper still allows a teacher to replace the final grade through the ordinary audited override. Core stores pre-grade manual feedback on the attempt, carries it into later grading, and records `feedback_teacher_authored`/`feedback_teacher_revised`; any original `PluginMcqAiEvaluation` remains immutable. Learner-facing headings and errors use neutral “Feedback” wording rather than identifying AI as the source.

## Contributor Workflow

Standalone MCQ gradebook results expose **Class overview** and reuse the Test aggregate report: correct choices, response counts, exact-answer accuracy, score statistics, and hoverable respondent names per choice.

When changing this plugin, update this detailed reference, the package README if its overview or routing changed, and `PROJECT_MEMORY.md` only when a durable invariant changed.
