export const codingExercisesDatabaseModule = {
  ...
  migrations: [
    ...
    {
      id: "202609190013_remove_rubric_names",
      statements: [
        `UPDATE "PluginCodingExerciseReferenceSolution"
         SET "privateConfig" = jsonb_set("privateConfig", '{aiFeedback}', ("privateConfig"->'aiFeedback') - 'rubricName')
         WHERE jsonb_typeof("privateConfig"->'aiFeedback') = 'object' AND ("privateConfig"->'aiFeedback') ? 'rubricName'`,
        `UPDATE "PluginBankCodingExerciseReferenceSolution"
         SET "privateConfig" = jsonb_set("privateConfig", '{aiFeedback}', ("privateConfig"->'aiFeedback') - 'rubricName')
         WHERE jsonb_typeof("privateConfig"->'aiFeedback') = 'object' AND ("privateConfig"->'aiFeedback') ? 'rubricName'`,
        `UPDATE "PluginCodingExerciseExecution"
         SET "aiFeedbackConfigSnapshot" = "aiFeedbackConfigSnapshot" - 'rubricName'
         WHERE jsonb_typeof("aiFeedbackConfigSnapshot") = 'object' AND "aiFeedbackConfigSnapshot" ? 'rubricName'`,
        `UPDATE "PluginCodingExerciseAiEvaluation"
    ...
    {
      id: "202609210010_test_regrade_evaluations",
      statements: [
