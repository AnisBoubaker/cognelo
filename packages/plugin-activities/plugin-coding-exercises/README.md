...
Course and bank authoring add a host-level **Grading** tab immediately after **Concepts**. It holds the deterministic/rubric weighting, general rubric, optional automatic-feedback controls, generated-test action, visible sample tests, and hidden tests. The plugin keeps one mounted authoring state and portals that grading portion into the host tab, so switching tabs cannot discard the combined public/private draft. Both Activity and Grading expose the same save/discard boundary.
