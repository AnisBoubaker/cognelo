...
At creation time, a Programming Exercise starts with the owning Subject's optional programming-language default in course, bank, course-Test, and reusable-bank-Test contexts. The creation layer overlays only the public `language` field on a fresh copy of the plugin defaults; it does not mutate registered defaults, rewrite existing activities, or replace the saved language of copied activities. When the Subject has no programming-language default, Python remains the plugin default.
