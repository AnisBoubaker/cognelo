...
export function normalizeParsonsGroups(value: unknown, lineCount: number) {
  ...
  return value
    .map((item) => {
      ...
      const startLine = typeof candidate.startLine === "number" ? Math.floor(candidate.startLine) : Number.NaN;
      const endLine = typeof candidate.endLine === "number" ? Math.floor(candidate.endLine) : Number.NaN;
...
function normalizeLegacyGroups(solution: string, value: LegacyConfig | undefined) {
  ...
  const groupMeta = new Map(
    legacyGroups
      .map((group) => {
        ...
        return [id, { id, label, orderSensitive: group.orderSensitive === false ? false : true }] as const;
      })
  ...
  lines.forEach((line, lineIndex) => {
    ...
    if (candidateId !== currentGroupId) {
      if (currentGroupId) {
        ...
        if (meta) {
          ranges.push({ ...meta, startLine, endLine: lineIndex - 1 });
        }
      }
  ...
  if (currentGroupId) {
    const meta = groupMeta.get(currentGroupId);
    if (meta) {
      ranges.push({ ...meta, startLine, endLine: lines.length - 1 });
    }
  }
...
function rebaseParsonsGroup(group: ParsonsGroup, changeStart: number, deletedCount: number, insertedCount: number) {
  ...
  if (deletedCount === 0) {
    if (changeStart < group.startLine) {
      return { ...group, startLine: group.startLine + delta, endLine: group.endLine + delta };
    }
    if (changeStart >= group.startLine && changeStart <= group.endLine + 1) {
      return { ...group, endLine: group.endLine + delta };
    }
    return group;
  }
  ...
  return { ...group, startLine: Math.max(0, nextStart), endLine: Math.max(0, nextEnd) };
}
...
function clampParsonsGroup(group: ParsonsGroup, lineCount: number) {
  ...
  return { ...group, startLine, endLine };
}
