# Web Design Coding Exercises Detailed Decisions

This document preserves detailed Web Design Coding Exercises decisions. Load only the sections relevant to the task.
...
- Standalone starter-file edits load and autosave through core `ActivityResponseDraft`; serialized saves prevent stale writes, activity-object identity refreshes do not reset current files, and final submission clears the draft. Test-embedded web-design activities stay on the Test execution host and `TestItemAttempt` autosave path; the Test shell remounts their workspace at item boundaries so editor state cannot cross into another question.
...
- A reusable bank Test owns an independent web-design bank child and must dispatch the same duplication, bank-to-course, course-to-bank, and deletion hooks for that child; it must never keep a live reference to the source activity.
...
- Standalone gradebook **Class overview** is teacher-only, renders the private reference bundle, and summarizes every enabled hidden Playwright test in its own hoverable pass/fail bar; the private bundle remains unavailable through student routes.
...
- Course/bank sync reuses `onCourseActivityCreatedFromBankVersion` for bank-to-course replacement and `onCourseActivityPublishedToBank` for course-to-bank replacement of private bundle/test rows. Core owns versions and permissions, locks bank-to-course retrieval after attempts, and continues to allow course-to-bank publication.
