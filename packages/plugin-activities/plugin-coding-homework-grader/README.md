...
Phase 14 added the first operational hardening slice. Student final ZIP submissions can include an `idempotencyKey` so browser/network retries reuse the existing submission instead of creating duplicates. Upload/extraction, analysis, and challenge generation append processing timeline entries to submission metadata, failed processing stores categorized retryable/non-retryable error details, and teachers can use `coding-homework-grader/reprocess` to retry unfinished/failed submissions without duplicating derived function/question rows. A true background worker remains a later production hardening step.
