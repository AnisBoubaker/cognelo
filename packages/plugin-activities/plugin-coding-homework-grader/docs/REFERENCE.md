# Coding Homework Grader Detailed Reference

This document is the detailed reference for the Coding Homework Grader plugin.
...
Phase 13 added gradebook integration and teacher manual grading. For summative assigned activities, final answer submission creates/submits a core `ActivityAttempt` linked to the plugin submission. Teachers can open the activity gradebook detail page, use **Review and grade**, inspect submitted file metadata, generated questions, student answers, and selected function source, then save score/feedback through the normal core gradebook override workflow.
...
When changing this plugin, update this detailed reference or the phased implementation plan, the package README if its overview or routing changed, and `PROJECT_MEMORY.md` only when a durable invariant changed.
