...
test.describe.serial("course, group, participant, attempt, and gradebook workflows", () => {
  ...
  test("teacher assigns a summative activity with the full attempt and grading policy", async ({ teacherPage: page }) => {
    ...
    await expect(settings.getByRole("tab", { name: "AI settings" })).toBeVisible();
    ...
    await expect(firstOverrideCheckbox).toBeVisible();
    expect((await firstOverrideCheckbox.boundingBox())?.width).toBeLessThan(30);
    await settings.getByRole("tab", { name: "General" }).click();
    await settings.getByLabel("Assessment mode").selectOption("summative");
    await settings.locator(".activity-summative-settings").getByRole("checkbox").first().check();
    await settings.getByRole("tab", { name: "Group assignment" }).click();
    const safeExamBrowserCheckboxes = settings.getByRole("checkbox", { name: "Require Safe Exam Browser", exact: true });
    await expect(safeExamBrowserCheckboxes).toHaveCount(1);
    await safeExamBrowserCheckboxes.first().check();
    await expect(safeExamBrowserCheckboxes).toHaveCount(2);
    await expect(safeExamBrowserCheckboxes.nth(1)).toBeChecked();
    await safeExamBrowserCheckboxes.first().uncheck();
    const pointsOverride = settings.getByRole("checkbox", { name: "Points possible" });
    await pointsOverride.check();
    await expect(settings.locator(".activity-override-field").filter({ hasText: "Points possible" }).getByRole("spinbutton")).toBeVisible();
    await pointsOverride.uncheck();
    await settings.getByRole("tab", { name: "General" }).click();
    await settings.locator(".activity-summative-settings").getByRole("checkbox").first().uncheck();
