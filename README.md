...
- **AI assessment responsibility is split deliberately**: core owns the course gate/model resolution, teacher trigger, grade/release/audit lifecycle, normalized `AiFeedbackResearchEvent` stream, and `GradeChallenge` workflow. A capable activity plugin owns complete activity configuration, prompt/rubric versioning, private raw evaluation artifacts, schema validation, sanitized feedback, any deterministic/AI score composition, and a teacher review renderer plus server-side revision validator. The detailed gradebook can review one learner or navigate every generated result; teacher edits change only the learner-visible normalized snapshot, preserve the immutable model artifact, and record grade/research audit events. Summative evaluation is invoked directly by a teacher and never starts from submission or the background-job worker; formative evaluation starts from the learner's explicit check/submit action. Learner-facing copy calls the result feedback/assessment feedback and does not disclose the generating or grading mechanism.
