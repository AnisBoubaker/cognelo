...
- Each subject edit page includes a visual knowledge-graph editor built with React Flow in its Knowledge graph tab. Every concept has a newline-delimited `skills` field containing observable learning goals or things the learner can perform. Concepts and their canvas positions are persisted per subject only when the main Subject form is saved. A directed edge originates at the concept that requires another concept and points to the required prerequisite; self-links, duplicate links, cross-subject links, and cycles are rejected. Multiple dependencies from one concept and multiple incoming dependencies are deterministically fanned across the four node handles, and selected edges are elevated for tracing through overlaps. Selecting a concept colors and fully emphasizes its direct prerequisite/dependent neighbors, keeps its complete directed chain of transitive prerequisites at full emphasis, and dims concepts outside those sets. The editor supports adding, editing, deleting, dragging, panning, zooming, a minimap, viewport controls, vertical workspace resizing, a horizontally resizable concept/skills inspector, unsaved-change protection, and restoring the last saved graph. The inspector divider supports pointer dragging and keyboard adjustment while preserving minimum widths for both panes. The subject detail page shows a tall read-only graph preview after its Activity banks and Courses panels; its viewport can still be panned and zoomed so larger graphs and routed edges remain inspectable.
...
- Every activity AI authoring panel offers `Use selected skills`, `Suggest skills`, and `Ignore skills`. Selected mode adds the current on-screen activity skill draft as generation constraints. Suggest mode ignores the current selection, maps the generated activity to exact skills from the subject catalog, and replaces only the unsaved Concepts-tab draft. Ignore mode neither reads nor changes that draft. Activity and Concepts tab panels remain mounted while switching, share one draft/save boundary, and a save from either tab persists activity content and knowledge selections together.
