...
describe("participant CSV import", () => {
  it("parses the fixed headerless student format with an optional fourth column", () => {
    ...
    expect(result.rows).toEqual([
      { line: 1, firstName: "Ada", lastName: "Lovelace", email: "ada@example.org", externalId: "ets-1" },
      { line: 2, firstName: "Grace", lastName: "Hopper", email: "grace@example.org", externalId: null }
    ]);
  });
  ...
  it("rejects header rows and alternate delimiters instead of guessing", () => {
    ...
    expect(semicolon.issues).toEqual([{ code: "invalid_column_count", line: 1, value: "1" }]);
  });
