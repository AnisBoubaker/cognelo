              ...
              <>
                ...
                {loading ? <p className="muted">{t("common.loading")}</p> : users.length ? (
                  <div className="table-list user-settings-list">
                    ...
                    {users.map((user) => <div className="table-row table-row-users" key={user.id}><strong>{user.firstName} {user.lastName}</strong><span>{user.email}</span><div className="metadata-badges">{user.roles.map((role) => <span className="metadata-badge" key={role.key}>{t(`roles.${role.key}`)}</span>)}</div><button className="secondary" type="button" onClick={() => openEdit(user)}>{t("common.edit")}</button></div>)}
                  </div>
