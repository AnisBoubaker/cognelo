...
- Subjects: shared curriculum containers with an explicit teaching language, an optional default programming language, subject-level material, activity banks, and subject-scoped knowledge graphs with draggable nodes and reconnectable arrow endpoints
...
Existing activities with no assignment rows initially appear assigned to every current group so opening and saving preserves the old effective behavior. Once explicit assignment state exists, real `CourseGroupActivity` rows are authoritative. Groups created later start unassigned, including when every earlier group was assigned. Formative activities record plugin checks/events for analytics without gradebook submissions, while summative activities expose plugin submission flows and create gradebook attempts/grades. Summative General and group-override settings configure the gradebook item policy, including points, pass/fail thresholds, attempt limits, and grade selection strategy.
...
- Subjects may also store a separate default programming language. When set, every newly created Programming Exercise in one of the Subject's courses or activity banks—including a locally created child inside a course or reusable bank Test—starts with that language instead of the plugin default. Existing activities and copied activities retain their saved configuration, and clearing the Subject default restores the plugin default for future creations.
