...
Shared cross-plugin frontend primitives live in `packages/activity-ui`. In addition to editors, renderers, notifications, and unsaved-navigation guards, it owns the responsive `EditActionBar` used by guarded edit drafts and the portal-based `ContextMenu` used by row/action menus. `MarkdownRenderer` is the sanitized platform rendering boundary for GitHub-flavored Markdown and KaTeX-compatible inline/display math, including `$$ ... $$`; authoring surfaces use the paired `RichTextEditor` instead of plugin-local textareas or WYSIWYG implementations. The editor renders formulas as protected, non-editable KaTeX widgets in Visual mode, preserves original delimited Markdown through visual round trips, and leaves existing-formula editing to its Markdown source tab. Complete delimited formulas entered or pasted as Visual text are preserved verbatim instead of being Markdown-escaped and become rendered widgets when Visual focus leaves. `ContextMenu` renders outside clipping containers, flips and clamps to the viewport, and centralizes dismissal behavior; new contextual menus must use it instead of page-local absolute positioning. The action bar is presentational and receives the owning form's dirty, saving, save, and discard state; it does not merge persistence boundaries or infer whether unrelated immediate mutations are saved.
