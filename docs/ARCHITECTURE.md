...
- **Subjects** own reusable curriculum context, a supported teaching-language locale used by subject AI generation, an optional default programming language for newly created Programming Exercises in their courses and banks, subject-level material, and a subject-scoped prerequisite knowledge graph. Programming-language choices are derived from the runtimes exposed by the configured Judge0 instance; a Subject can instead declare multiple languages. Missing or multiple-language defaults leave each new exercise unconfigured until its author chooses a language. Manual graph deletions remain immediate, impact-analyzed operations. AI generation preserves retained stable identities and may attach an exact concept/skill deletion disclosure to the combined Subject draft; core transactionally retires only those disclosed omissions and removes their current bank/course activity mappings, while historical version snapshots remain immutable and all other omissions remain guarded.
- **Activity AI authoring** receives knowledge alignment through a host-owned shared context. Every generation mode receives the complete subject concept/skill catalog as a curriculum boundary. Generators can additionally consume the live unsaved skill draft as specific targets, request catalog-constrained skill suggestions for their generated draft, or ignore the selection draft without reading or mutating it. The host keeps tab panels mounted and combines activity content with knowledge selections at the persistence boundary. Activity types with a dedicated grading authoring surface can opt into the host **Grading** tab after **Concepts**; Programming Exercises keep one mounted plugin state and portal their grading controls into that panel so tab navigation does not split the save boundary.
- **Assessment feedback and AI grading** use a cross-plugin orchestration boundary. Course metadata holds a dedicated AI-feedback enablement switch and AI connection, separate from student support. Activity definitions advertise `supportsAiFeedback` and `supportsAiFeedbackGrading`; server plugins register an `aiFeedback.evaluateAttempt` handler returning an immutable feedback reference/version/hash, sanitized student feedback, and an optional grading result. Feedback-capable plugins also register `aiFeedback.teacherReview` submission, draft, and revision handlers plus an activity-definition renderer key. The detailed gradebook supplies one **Review and grade** surface with individual or whole-class navigation: the plugin renders the answer and validates editable narrative and rubric fields, while the host provides the audited final-grade override. Drafts may contain an activity's configured rubric even before automatic evaluation, allowing the same plugin-owned form and grading policy to support teacher-authored feedback. Programming Exercise rubric and assessment-feedback model prompts resolve the Subject teaching language server-side, not from the viewer's interface locale. Formative plugin routes call the evaluator immediately after an explicit learner check/submit. Summative submission only persists work; an authorized teacher invokes the direct attempt-evaluation endpoint from the gradebook, with no scheduled scan or background-job execution. Core records feedback-only results without changing deterministic grades, records plugin grading results through the normal grade lifecycle, and withholds all summative feedback until release. Learner-facing copy remains mechanism-neutral.
- **Feedback persistence and research** deliberately separate private AI artifacts and normalized learner-visible data. Each plugin stores complete request/rubric snapshots, provider response, parsed output, latency, hashes, score components, and failure details in plugin-owned tables; coding-exercise submission rows preserve the original private rubric/test result, while teacher-triggered corrections append new test or model evaluations using current authoring state. Feedback authored before grading is stored on `ActivityAttempt.metadata.teacherFeedback`, copied into a later grade unless that grading call supplies generated feedback, and recorded as `feedback_teacher_authored`. A later teacher edit updates that attempt copy plus the selected grade snapshot when present, preserves immutable model artifacts, records the previous/next snapshot in `GradeEvent`, and appends `feedback_teacher_revised` to `AiFeedbackResearchEvent`. A feedback plugin may return a grading result when an editable rubric score changes; core records it through the ordinary audited regrade service and appends `feedback_teacher_grade_adjusted`, while that review's deterministic component and configured weights remain fixed. Generated feedback revisions are blocked after that version has been challenged so the contested snapshot remains stable. Core also stores bounded append-only research envelopes for requests, completions/failures, grade recording, release/hide/view, Test parent recomputation, and challenge outcomes. The manager-only research route pseudonymizes participant/user/actor/attempt identifiers by default. It is an operational research extract, not yet a consent-aware production export; consent filtering plus retention/anonymization policy remain required hardening.
...
- **Activity banks** own reusable activity authoring and version history for a subject. `ActivityBankFolder` provides bank-scoped nested organization; folders and activities have a shared logical sibling order, while a dedicated activity-placement mutation keeps drag-and-drop changes outside the authored-content/version path. Reusable Tests are core bank activities with a normalized `BankTest` graph of hidden, independently owned child bank activities; each published shell version has an immutable ordered composition manifest referencing exact child versions. Bank/course Test transfer deep-copies the complete graph and invokes ordinary child lifecycle hooks, so deleting a source activity, bank Test, or bank never mutates existing copies. Folder deletion preserves top-level activities by moving every item in the deleted subtree to the bank root. The detail response derives per-concept activity counts from normalized top-level bank links, and the client applies inclusive concept filtering while retaining matching ancestor folders. Bank management is capability-gated (currently owner/admin), subject changes are allowed only while empty, and populated-bank deletion either transfers activities to a writable same-subject bank or cascades bank content after explicit confirmation while preserving detached course-local copies.
