...
- **Bank activity lifecycle hooks** keep generic duplicate operations plugin-neutral. Core creates an independent version-1 bank record, while activity plugins copy any private bank-owned authoring rows through `onBankActivityDuplicated`; moving retains the existing activity identity and therefore needs no plugin data migration.
