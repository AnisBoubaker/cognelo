...
- Core owns attempts, attempt limits, gradebook lifecycle, release visibility, audited regrades, research events, and grade challenges. Challenge queues must enter grading through the same plugin **Review and grade** surface as the gradebook; the challenge panel only sends the response and may notify the student by guarded system email. Plugins own activity-specific evaluation and sanitized feedback contracts. A plugin-specific released learner report must independently recheck learner ownership and release state and expose only bounded student-safe evidence.
