...
- Coding Homework Grader Phase 14 first hardening slice is implemented. Student ZIP submissions support idempotency keys, processing steps append plugin-owned timeline metadata, failures are categorized with retryability, and teachers can reprocess unfinished/failed submissions through a plugin route. Processing is still request-driven; a true background queue remains future work.
